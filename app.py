...
def plot_data(stock_symbols, time_scale, all_stock_data):
    metrics = ['VWAP', 'HIGH', 'LOW', 'OPEN', 'CLOSE']

    # Create subplots with 3 rows and 1 column
    fig, axes = plt.subplots(nrows=3, ncols=1, figsize=(15, 15))
    fig.suptitle(f'Stock Metrics Over Time ({time_scale} scale)')

    # Flatten the 3x1 array of subplots for easier indexing
    axes = axes.flatten()
    ...
    for i, stock_symbol in enumerate(stock_symbols):
        ...
        # Plot 'LOW' and 'HIGH' in the first subplot
        axes[0].plot(stock_symbol_data['DATE'], stock_symbol_data['LOW'], label=f'{stock_symbol} - LOW', alpha=0.5)
        axes[0].plot(stock_symbol_data['DATE'], stock_symbol_data['HIGH'], label=f'{stock_symbol} - HIGH', alpha=0.5)

        # Plot 'OPEN' and 'CLOSE' in the second subplot
        axes[1].plot(stock_symbol_data['DATE'], stock_symbol_data['OPEN'], label=f'{stock_symbol} - OPEN', alpha=0.5)
        axes[1].plot(stock_symbol_data['DATE'], stock_symbol_data['CLOSE'], label=f'{stock_symbol} - CLOSE', alpha=0.5)

        # Plot 'VOLUME' and 'NO OF TRADES' in the third subplot
        axes[2].plot(stock_symbol_data['DATE'], stock_symbol_data['VOLUME'] / 1e5, label=f'{stock_symbol} - VOLUME (10^5)', alpha=0.5)
        axes[2].plot(stock_symbol_data['DATE'], stock_symbol_data['NO OF TRADES'] / 1e3, label=f'{stock_symbol} - NO OF TRADES (10^3)', alpha=0.5)

    # Set subplot titles and labels
    axes[0].set_title('Low and High Prices')
    axes[1].set_title('Open and Close Prices')
    axes[2].set_title('Volume (10^5)')
